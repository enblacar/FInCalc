...
def main():
    ...
    st.set_page_config(layout = "wide", 
                       page_icon="random", 
                       page_title="Finance Calculators")

    # Define pages
    p1 = st.Page(page="./page_directory/01_compound_interest.py", 
    ...
    with st.expander("**Theme options**"):
        ...
        with st.container():
            ...
            with col1:
                fontsize_value = st.slider("**Font size**", 
                                           min_value = 8, 
                                           max_value = 24, 
                                           value = 16, 
                                           step = 1, 
                                           key = "fontsize_slider", 
                                           help = "Controls the overall font size across all elements in the plots.")

                add_palette_ends = st.toggle("B/W ends", 
                                             help = "Adds white and black ends to continuous color palettes so that the range of colors is concentrated on the middle values of the scale.")
            with col2:
                ...
                with st.container():
                    ...
                    with col1:
                        discrete_colors = st.selectbox("**Qualitative**", 
                                                       list(plotly_qualitative.keys()), 
                                                       key = "qualitative_colors", 
                                                       index = list(plotly_qualitative.keys()).index("Prism"), 
                                                       help = "Qualitative color palette used when plotting categorical data.")

                    with col2:
                        display_palette_as_gradient(plotly_qualitative[discrete_colors], 
                                                    continuous = False)
                    ...
                    col3, col4 = st.columns([2, 8], 
                                            ...
                                            vertical_alignment = "center")
                    with col3:
                        continuous_colors = st.selectbox("**Sequential**", 
                                                         list(plotly_sequential.keys()), 
                                                         key = "sequential_colors", 
                                                         index = list(plotly_sequential.keys()).index("YlGnBu"), 
                                                         help = "Sequential color palette used when plotting continuous data.")
                    ...
                    col5, col6 = st.columns([2, 8], 
                                            ...
                                            vertical_alignment = "center")
                    with col5:
                        divergent_colors = st.selectbox("**Diverging**", 
                                                        list(plotly_divergent.keys()), 
                                                        key = "diverging_colors", 
                                                        index = list(plotly_divergent.keys()).index("RdBu_r"), 
                                                        help = "Diverging palette used when plotting continuous, centered data.")
    ...
    pg = st.navigation(dict(Calculators=[p1]))
    pg.run()
